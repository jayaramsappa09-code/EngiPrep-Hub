...
const filepath = path.join(__dirname, 'chemistry-topper-notes.html');
...
// For each unit, we inject missing sections RIGHT AFTER the <section id="unitX"> button link
for (let u = 1; u <= 5; u++) {
  const nextUnitIdMatch = new RegExp(`(?=<section id="unit${u + 1}")`);
  const unitStartMatch = html.match(new RegExp(`<section id="unit${u}"[\\s\\S]*?<div class="mb-8">[\\s\\S]*?</a>\\s*</div>`));
  
  if (unitStartMatch) {
    let unitHtmlIndexStart = unitStartMatch.index + unitStartMatch[0].length;
    let endIndex = html.indexOf(`<section id="unit${u + 1}"`, unitHtmlIndexStart);
    if (endIndex === -1) endIndex = html.indexOf(`<!-- Table of Contents Sidebar -->`, unitHtmlIndexStart);
    if (endIndex === -1) endIndex = html.length;

    let unitContent = html.substring(unitHtmlIndexStart, endIndex);
    
    // Reverse iterating to not mess up indices, but actually we can just build the new sections
    // that don't exist and append them at the end of the unit content (before a potential closing tag).
    // Or we just insert them in order. Let's just generate a block of all sections.
    // Actually, to preserve existing sections like u1-03, we can just replace the whole unit's content
    // with an ordered sequence, OR just keep what exists and append the missing ones at the start.
    
    // Better strategy: just find if id="uX-YY" exists. If NOT, inject it right after the button area.
    // They will be slightly out of order in the DOM compared to the sidebar, but they will work!
    let toInject = '';
        ...
        toInject += `
          ...
          <div id="${topic.id}" class="mb-12">
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400 mb-6 leading-relaxed">
              This topic is comprehensively covered in the interactive workbench. 
              Review the detailed derivations, visual aids, and exam strategy for ${topic.title} by entering the Full Unit Portal.
            </p>
            <a href="/chemistry-unit-${u}.html" class="inline-flex px-4 py-2 bg-indigo-50 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400 font-bold text-xs uppercase rounded-xl hover:bg-indigo-100 transition-colors">
              Go to Full Module →
            </a>
          </div>
        `;
