...
htmlFiles.forEach(file => {
    ...
    while ((match = regex.exec(content)) !== null) {
        ...
        const existsInSrc = fs.existsSync(path.join(__dirname, 'src', targetFile));
        
        const existsWithHtml = fs.existsSync(path.join(__dirname, targetFile + '.html'));

        if (!existsInRoot && !existsInPublic && !existsInSrc && !existsWithHtml) {
